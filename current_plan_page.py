...
from plans import save_plan_to_xml, load_plan_from_xml
...
class Current_Plan_page(ctk.CTkFrame):
    ...
    def download_and_save(self):
        ...
        try:
            ...
            for row in self.current_plan[1:]:  # skip header
                plan_data["drills"].append({
                    "name": row[1],
                    "duration": row[0],
                })
    ...
    def generate_plan(self, team_age, analysis):
        ...
        for row in analysis[1:]: #Skips header row
            ...
            #Freethrows is a drill_name not a tag, so this is a special case kept separate from main loop
                ...
                if any(tag in drill.drill_tags for tag in tags):
                    ...
                    if team_age in drill.drill_age:
                        ...
                        if added_drills >= amount:
                            break
        total_time = 12 #starts at 12 to count offences and freethrows which is added last

        #Calulates total time for training plan
        for row in plan_contents[1:]:
            total_time += int(row[0])

        #Changes length of training session based on team age
        if team_age in ["U8s", "U10s"]:
            min_duration = 60  #Younger age groups tend to go for 60 min
            max_duration = 70
        else:
            min_duration = 90   #Most represnetative trainng sessions go for 90 min
            max_duration = 100
        ...
        if total_time < min_duration:
            for drill in self.drill_manager.drills:
                ...
                if team_age in drill.drill_age:
                        ...
                        if min_duration <= total_time < max_duration:
                            break
    ...
    def __init__(self, parent, controller=None):
        ...
        def edit_plan():
            ...
            edit_plan_popup.geometry("650x680")
            ...
            drills_dropdown = ctk.CTkOptionMenu(edit_plan_popup, height=45, values=drill_names, text_color="white", font=("Abadi", 17), fg_color="#FF7A53", button_color="#FF7A53", button_hover_color="#c7c7c7", corner_radius=10,)
            drills_dropdown.grid(row=0, column=1, pady=5, padx=5, sticky="ew")

            add_button = ctk.CTkButton(edit_plan_popup, height=45, text="Add Drill", font=("Abadi", 17), text_color="white", corner_radius=10, fg_color="#FF7A53", hover_color="#c7c7c7", command=add_drill)
            add_button.grid(row=0, column=2, pady=5, padx=5, sticky="ew")

            save_button = ctk.CTkButton(edit_plan_popup, height=45, text="Save", font=("Abadi", 17), text_color="white", hover_color="#c7c7c7", fg_color="#7ec76b", corner_radius=10, command=save_plan)
